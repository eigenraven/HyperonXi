...
pragma(inline, true) void criticalFailure()()
{
	while (1)
	{
	}
...
pragma(inline, true) ulong unitMultiplier(ulong mul)(ulong val)
{
	return val * mul;
}

alias KB = unitMultiplier!(1024);
alias MB = unitMultiplier!(1024 * 1024);
alias GB = unitMultiplier!(1024 * 1024 * 1024);
alias TB = unitMultiplier!(1024L * 1024L * 1024L * 1024L);
...
/// Tests if low<=val<=high
bool inRange(T)(T val, T low, T high)
{
	return (low <= val) && (val <= high);
}

/// Clamps val to [low,high]
T clamp(T)(T val, T low, T high)
{
	if (val < low)
		return low;
	if (val > high)
