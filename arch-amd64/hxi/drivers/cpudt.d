/// CPU descriptor tables (IDT, GDT, etc)
module hxi.drivers.cpudt;

import barec;
import hxi.memory.allocators;
import hxi.memory.paging;
import hxi.memorymap;
import hxi.output;
...
void initializeCpuDT()
{
    ...
    ulong interruptStackPointer = cast(ulong)(ActivePageTable.multiMapAddress(
            cast(void*) HxiFixedAddress.KernelRing0StacksBase, 6, MapMode.kernelPage, true));
    ulong kernelStackPointer = cast(ulong)(ActivePageTable.multiMapAddress(cast(
            void*) HxiFixedAddress.KernelRing0StacksBase + 0x6000, 6, MapMode.kernelPage, true));
    ulong userStackPointer = cast(ulong)(ActivePageTable.multiMapAddress(cast(
            void*) HxiFixedAddress.KernelRing3StacksBase, 6, MapMode.kernelPage, true));
    ...
    log(LogLevel.Info, "Reloading GDT with TSS");
